...
const Home_Trending = () => {
  const productsTrend = data.products.filter(
    (trend) => trend.category === "trend"
  );
  const productsSeller = data.products.filter(
    (trend) => trend.category === "seller"
  );
  const productsFeature = data.products.filter(
